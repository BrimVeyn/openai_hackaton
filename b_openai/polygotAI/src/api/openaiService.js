...
export function generate_flashcard(args, setShow, setContent, setCards, addCard) {
  //           default:
  setShow(false);
...
export async function getAssistantResponse(userMessage, setShow, setContent, setCards, addCard) {
  try {
    const assistant = await openai.beta.assistants.retrieve("asst_GyI3Cd5WyjmUpgna5VaIVwfI");
    const thread = await openai.beta.threads.create();
    ...
    if (run.status === "completed") {
      ...
      switch (funcName) {
        case 'generate_flashcard':
          await generate_flashcard(funcArgs, setShow, setContent, setCards, addCard);
          return "generate_flashcard()";
        default:
          break;
      }
